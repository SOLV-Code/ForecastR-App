---
title: "Long Word Report - Model Comparison"
output: 
  word_document: 
    fig_caption: yes
params:
...
caption.use <- paste0("Table 3: Cumulative Model Ranks for ",params$FCYear, " Forecast of ",params$Abundance, " for ",params$Stock," ",params$Species,". Rows shows forecasts for highest-ranked models by age and alternative total forecasts. Total column shows the rank of the total forecast using that model. Sum is the sum of ranks for age-specific forecasts using that model."    )


table.in <- params$Table_CumulRanks %>% rename(Sum = rank.sum)
names(table.in) <- gsub(".rank","",names(table.in))
...
caption.use <- paste0("Table 4: ",params$Int_Type," Intervals for ", params$FCYear, " Forecast of ",params$Abundance, " for ",params$Stock," ",params$Species,". Each row shows age-specific estimates for one of the alternative models."    )

kable(params$Table_Multi_Int_FC, 
		caption = caption.use,
		format = "pandoc")
```
...
caption.use <- paste0("Table 5: Retrospective Performance Measures for ",params$FCYear, " Forecast of ",params$Abundance, " for ",params$Stock," ",params$Species,". TEXT."    )

table.in <- params$Table_RetroPM
...
caption.use <- paste0("Table 6: Retrospective Model Ranks for ",params$FCYear, " Forecast of ",params$Abundance, " for ",params$Stock," ",params$Species,". TEXT."    )

table.in <- params$Table_RetroRanks 

names(table.in) <- gsub("\\.","_",names(table.in))

table.in <- table.in %>% select(Age,Model,contains("E_rank"),rank_avg)  %>% dplyr::rename(Avg = rank_avg)
names(table.in) <- gsub("_rank","",names(table.in))
