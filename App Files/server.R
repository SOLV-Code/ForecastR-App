...
# This pulls in the code for the new modules from the main repo
    ...
    data.file <- reactive({
	    ...
	    if(input$data.source == "Sample 2 - No Ages"){data.file.tmp <- read.csv("DATA/NoAgeSampleFile.csv",stringsAsFactors=FALSE)}
		  if(input$data.source == "File") {data.file.tmp <- read.csv(inFile$datapath, stringsAsFactors=FALSE)}
			...
			data.use  <- data.use[,colSums(!is.na(data.use)) >0]
			...
			if(input$MainTab == "precheck"){
				yrs.window <- input$yr.range.precheck[1]:max(data.use$Run_Year)
			...
			# NEED TO FIX THIS FOR NEW CUSTOM REPORT
...
ages.list.short <-  reactive({
	ages.list.full <- ages.list()
	if(length(ages.list.full )>1){ages.vec.short <- ages.list.full[-c(1,2)]}
	if(length(ages.list.full )==1){ages.vec.short <- c("Not Applicable")}

	return(ages.vec.short)
})



axis.label.default <-  reactive({
	label.def <- settings.basic()[["Abundance"]]
	return(label.def)
	})
